...
std::shared_ptr<Curve> BsplineCurve::GetCurveFromPoints(const vector<Vector3d> &pnts, const GLdouble *const color, const GLdouble width) const
{
...
std::shared_ptr<Curve> GetBsplineCurveFromPoints(const vector<Vector3d> &pnts, int ord, const GLdouble *const color, GLdouble width)
{
    ...
    return std::shared_ptr<BsplineCurve>(new BsplineCurve(ord, &new_cps[0], new_ncpnt, &new_knots[0], color, width));
}
...
void BsplineCurve::AddKnot(const double t)
{
    ...
    {
        // 各値設定
        _nknot = (int)new_knot.size();
        SetKnotVector(&(new_knot[0]), (int)new_knot.size());

        _ncpnt = (int)new_cps.size();
        SetControlPoint(&(new_cps[0]), (int)new_cps.size());
...
void BsplineCurve::GetDevidedCurves(std::vector<double> &params, std::vector<std::shared_ptr<Curve>> &devided_curves)
{
    ...
    for (const auto &t : params)
    {
        ...
        int t_cnt_now = (int)std::count(_knot.begin(), _knot.end(), t);
    ...
    for (int i = 0, s = (int)params.size(); i < s + 1; ++i)
    {
        ...
        // 分割最後方のノット位置を取得
        for (int j = 0, nknot = (int)_knot.size(); j < nknot - 1; ++j)
        {
            if (i != s)
            {
        ...
        // ノットの正規化
        AdjustKnotVector(split_knot, _ord, 0, 5);
