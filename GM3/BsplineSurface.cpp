...
void BsplineSurface::AddKnot(const ParamUV direction, const double param)
{
    ...
    // 3. 曲面データの調整
    {
        ...
        {
            // 各値設定
            _nknotU = (int)new_knot.size();
            SetKnotVector(&(new_knot[0]), (int)new_knot.size(), _knotU);
            ...
            _ncpntU = (int)new_cps[0].size();
            SetControlPoint(&(new_cps_flat[0]), (int)new_cps_flat.size());
        }
        else
        {
            // 各値設定
            _nknotV = (int)new_knot.size();
            SetKnotVector(&(new_knot[0]), (int)new_knot.size(), _knotV);
    ...
    std::vector<std::shared_ptr<Surface>> &devided_surfs, const GLdouble *const color)
{
    ...
    for (const auto &param : params)
    {
        ...
        int param_cnt_now = (int)std::count(knot.begin(), knot.end(), param);
    ...
    // 分割位置にノットを足して分割
    for (int i = 0, s = (int)params.size(); i < s + 1; ++i)
    {
        if (i == 0)
            t_start = 0;
        ...
        // 分割最後方のノット位置を取得
        for (int j = 0, nknot = (int)knot.size(); j < nknot - 1; ++j)
        {
            if (i != s)
            {
    ...
    std::vector<Vector3d>& on_params, std::vector<Point<double>>& uv_params) const
{
    ...
    while (true)
    {
        ...
        std::vector<NearestPointInfoS> nearest_pts;
        {
            for (auto& pts : c_pts)
            {
                nearest_pts.push_back(this->GetNearestPointInfoFromRef((Vector3d)pts));
            }
        }
        // 近似トレランスを計算
        {
        ...
        {
            Point<double> uv_param(pnt.paramU, pnt.paramV);
...

        {
            ...
            // UV曲線を取得
            auto uv_spline_curve = static_pointer_cast<NurbsCurve>(GetBsplineCurveFromPoints(uv_pts, 4, Color::green, 2));
            ...
            for (auto& pnt : pts_on_uv_curve)
                pts_on_surf.push_back(this->GetPositionVector(((Vector3d)pnt).X, ((Vector3d)pnt).Y));
            auto curve_on_surf = static_pointer_cast<NurbsCurve>(GetBsplineCurveFromPoints(pts_on_surf, 4, Color::blue, 2));

            // UV曲線から生成した面上線と元の曲線が近いかチェック
            if (curve->IsDifferentAtLeast(curve_on_surf.get(), tole))
            {
                cout << seg_split << endl;

                on_params = pts_on_surf;
                return std::make_shared<Edge>(curve_on_surf, uv_spline_curve);
            }
        }
